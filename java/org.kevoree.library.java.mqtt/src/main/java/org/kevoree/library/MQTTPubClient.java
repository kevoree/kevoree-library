	...
	public void stop() {
		if (connection != null) {
			connection.kill(new org.fusesource.mqtt.client.Callback<Void>() {
				...
				@Override
				public void onFailure(Throwable throwable) {
					Log.error(context.getInstanceName() + " failed to close connection with " + host + ":" + port,
							throwable);
				}
			});
		}
	...
	public void jsonPub(String jsonMsg) {
		if (this.connection != null) {
			...
			try {
				...
				Log.error("{} unable to parse incoming JSON message, message will not be published",
