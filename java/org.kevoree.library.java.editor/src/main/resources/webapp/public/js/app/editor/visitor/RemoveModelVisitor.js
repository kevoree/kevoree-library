define(
  ...
  function (KGroup, KInputPort, KOutputPort, Kevoree) {
      ...
      if (instance != null) {
        ...
        for (var i in wires) {
          var grp = this._model.findGroupsByID(wires[i].getOrigin().getName()),
            fragDics = wires[i].getOrigin().getFragmentDictionaries();
          
          for (var j=0; j < fragDics.length; j++) {
            if (fragDics[j].getName() === node.getName()) {
              var values = fragDics[j].getValues();
              for (var k=0; k < values.length; k++) {
                if (values[k]._instance) fragDics[j]._instance.removeValues(values[k]._instance);
              }
            }
          }
    ...
    RemoveModelVisitor.prototype.visitDictionary = function (dict) {}
    
    RemoveModelVisitor.prototype.visitFragmentDictionary = function (dict) {
      if (dict._instance && dict._instance != null && dict._instance.eContainer()) {
        dict._instance.eContainer().removeFragmentDictionary(dict._instance);
      }
    }
